    ...
    private void doOperationLog(OperationLog operationLog) {
        ...
        switch (operation) {
            ...
            case ADD_DOCUMENT:
                ...
                allIndexService.insertDocument(documentTuple.getT1(), documentTuple.getT2());
                break;
            case BULK_INSERT:
                Tuple<String, List<Document>> documentsTuple = (Tuple<String, List<Document>>) operationLog.getData();
                allIndexService.bulkInsert(documentsTuple.getT1(), documentsTuple.getT2());
                break;
            case DELETE_DOCUMENT:
                Tuple<String, Long> docIdTuple = (Tuple<String, Long>) operationLog.getData();
                allIndexService.deleteDocument(docIdTuple.getT1(), docIdTuple.getT2());
                break;
            case BULK_DELETE:
                Tuple<String, List<Long>> docIdsTuple = (Tuple<String, List<Long>>) operationLog.getData();
                allIndexService.bulkDelete(docIdsTuple.getT1(), docIdsTuple.getT2());
                break;
            case DELETE_BY_QUERY:
                Tuple<String, QueryBuilder> queryBuilderTuple = (Tuple<String, QueryBuilder>) operationLog.getData();
                allIndexService.deleteByQuery(queryBuilderTuple.getT1(), queryBuilderTuple.getT2());
                break;
        }
    }
